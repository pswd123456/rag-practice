...
def generate_testset_pipeline(db: Session, testset_id: int, source_doc_ids: List[int], generator_model: str = "qwen-max"):
    """
    根据指定的源文档生成测试集
    """
    ...
    try:
        logger.info(f"开始为 Testset {testset_id} 生成数据，源文档ID: {source_doc_ids}，使用模型: {generator_model}")
        ...
        # 2. 初始化 Generator
        # [修改] 使用传入的 generator_model 初始化 LLM
        generator_llm = setup_llm(generator_model) 
        # 为了生成高质量的问题，通常保持 Embedding 稳定
        ...
        langfuse.create_dataset(
            ...
            metadata={"testset_id": testset_id, "source": "rag-practice", "generator_model": generator_model}
        )
        ...
        # [修改] 在描述中增加模型信息
        testset.description = f"Generated by {generator_model}. Size: {len(df)}"
